...
function Interview({
  ...
  const pauseStatsRef = useRef({ pauseCount: 0, longestPauseSeconds: 0 });
  ...
  function getAverageVideoMetrics() {
    ...
    return {
      ...
      cheatingFlags,
      stressIndicators: [
        ...(stats.lookingAwayCount > 8 ? ["frequent looking away"] : []),
        ...(stats.faceLostCount > 5 ? ["face visibility interruptions"] : []),
        ...(stats.lookingDownCount > 8 ? ["looking down repeatedly"] : [])
      ]
    };
  }
  ...
  function startSpeechRecognition() {
    ...
    recognition.onstart = () => {
      ...
      pauseStatsRef.current = { pauseCount: 0, longestPauseSeconds: 0 };
    };
    ...
    recognition.onresult = (event) => {
      ...
      const now = Date.now();
      const gapSeconds = lastSpeechTimeRef.current ? (now - lastSpeechTimeRef.current) / 1000 : 0;
      if (gapSeconds > 2.2) {
        pauseStatsRef.current.pauseCount += 1;
        pauseStatsRef.current.longestPauseSeconds = Math.max(
          pauseStatsRef.current.longestPauseSeconds,
          Math.round(gapSeconds * 10) / 10
        );
      }
  ...
  function calculateAudioMetrics(text) {
    ...
    const fillers = ["um", "uh", "umm", "like", "actually", "basically", "so", "you", "know"];

    const fillerCount = words.filter((word) =>
      fillers.includes(word.toLowerCase().replace(/[^a-z]/g, ""))
    ).length;

    const elapsedSeconds = answerStartTimeRef.current
      ? Math.max(1, (Date.now() - answerStartTimeRef.current) / 1000)
      : Math.max(1, typingMetrics.timeTakenSeconds || 1);

    const speakingRateWPM = Math.round((words.length / elapsedSeconds) * 60);
    const pauseCount = pauseStatsRef.current.pauseCount;
    const longestPauseSeconds = pauseStatsRef.current.longestPauseSeconds;

    let confidenceScore = 92;
    confidenceScore -= fillerCount * 4;
    confidenceScore -= pauseCount * 5;
    if (longestPauseSeconds > 5) confidenceScore -= 10;
    if (words.length < 20) confidenceScore -= 15;
    if (speakingRateWPM > 0 && speakingRateWPM < 70) confidenceScore -= 8;
    if (speakingRateWPM > 180) confidenceScore -= 8;
    confidenceScore = Math.max(25, Math.min(100, Math.round(confidenceScore)));

    let communicationClarityScore = 90;
    communicationClarityScore -= fillerCount * 3;
    communicationClarityScore -= pauseCount * 3;
    if (speakingRateWPM > 0 && (speakingRateWPM < 70 || speakingRateWPM > 180)) {
      communicationClarityScore -= 10;
    }
    communicationClarityScore = Math.max(25, Math.min(100, Math.round(communicationClarityScore)));

    const averagePitchHz = 150 + Math.min(80, fillerCount * 4 + pauseCount * 6);
    const pitchVariation = Math.min(60, Math.round(Math.abs(speakingRateWPM - 120) / 2 + fillerCount * 3));
    const toneLabel = confidenceScore >= 75 ? "steady" : confidenceScore >= 55 ? "hesitant" : "nervous";
  ...
  <div className="start-panel">
      ...
      Begin Question
          ...
          {questionObj && (
            <div className="question-box">
              ...
              <p className="why">{questionObj.encouragement}</p>
    ...
    >
      🔊 Read Question
              ...
              {questionObj.adaptiveReason && (
                <p className="why">Adaptive reason: {questionObj.adaptiveReason}</p>
              )}
            </div>
          )}
          ...
          <div className="metrics">
            ...
            {interviewMode === "speech" && (
              <>
                ...
                <p>Clarity estimate: {audioMetrics.communicationClarityScore}/100</p>
                <p>Pauses: {audioMetrics.pauseCount}</p>
                <p>Longest pause: {audioMetrics.longestPauseSeconds}s</p>
                <p>Speaking rate: {audioMetrics.speakingRateWPM} WPM</p>
                <p>Tone estimate: {audioMetrics.toneLabel}</p>
                <p>Pitch variation estimate: {audioMetrics.pitchVariation}</p>
              </>
            )}
